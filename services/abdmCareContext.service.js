...
async function buildPatientCareContexts(patientId) {
  ...
  const [appointments, prescriptions, labs, radiology, discharges, invoices, immunizations, documents] = await Promise.all([
    Appointment.find({ patient_id: patient._id }).sort({ appointment_date: -1 }).limit(100).lean(),
    Prescription.find({ patient_id: patient._id }).sort({ issue_date: -1 }).limit(100).lean(),
    LabReport.find({ patient_id: patient._id }).sort({ report_date: -1 }).limit(100).lean(),
    RadiologyRequest.find({ patientId: patient._id }).sort({ requestedDate: -1 }).limit(100).lean(),
    DischargeSummary.find({ patientId: patient._id }).sort({ dischargeDate: -1 }).limit(50).lean(),
    Invoice.find({ patient_id: patient._id }).sort({ created_at: -1 }).limit(100).lean(),
    Immunization.find({ patientId: patient._id }).sort({ occurrenceDate: -1 }).limit(100).lean(),
    ClinicalDocument.find({ patientId: patient._id }).sort({ documentDate: -1 }).limit(100).lean()
  ]);
...
async function groupedForAbdm(patientId) {
  const patient = await Patient.findById(patientId);
  if (!patient) throw new Error('Patient not found');
  const contexts = await AbdmCareContext.find({ patientId }).sort({ createdAt: -1 }).lean();
