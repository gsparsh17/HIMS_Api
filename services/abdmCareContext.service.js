...
const {
  assertAbdmExchangeEligible
} = require('./abdmExchangeEligibility.service');
...
async function ensurePatientReference(patient) {
  ...
  await Patient.updateOne(
    ...
    { 'abha.patientReference': reference }
  );
  return reference;
}

function hospitalScopedRecords(patient, records) {
  if (!patient?.hospitalId) {
    throw new Error(
      'Patient hospitalId is required before building ABDM care contexts'
    );
  }
...
async function upsertContext({
  ...
  const scopedRecords = hospitalScopedRecords(patient, records);
  ...
  if (existing) {
    ...
    existing.records = scopedRecords;
...
async function buildPatientCareContexts(patientId) {
  ...
  const [
    ...
    Prescription.find({ patient_id: patient._id })
      .sort({ issue_date: -1 })
      .lean(),
    LabReport.find({ patient_id: patient._id })
      .sort({ report_date: -1 })
      .lean(),
    RadiologyRequest.find({ patientId: patient._id })
      .sort({ requestedDate: -1 })
      .lean(),
    DischargeSummary.find({ patientId: patient._id })
      .sort({ dischargeDate: -1 })
      .lean(),
    Invoice.find({ patient_id: patient._id })
      .sort({ created_at: -1 })
      .lean(),
    Immunization.find({ patientId: patient._id })
      .sort({ occurrenceDate: -1 })
      .lean(),
    ClinicalDocument.find({ patientId: patient._id })
      .sort({ documentDate: -1 })
      .lean(),
    Vital.find({ patient_id: patient._id })
      .sort({ recorded_at: -1 })
      .lean(),
    IPDVitals.find({ patientId: patient._id })
      .sort({ recordedAt: -1 })
      .lean()
  ]);
  ...
  for (const appointment of appointments) {
    contexts.push(
      await upsertContext({
        ...
        display: `OP Consultation ${displayDate(
          appointment.appointment_date || appointment.createdAt
        )}`,
  ...
  for (const prescription of prescriptions) {
    contexts.push(
      await upsertContext({
        patient,
        hiType: 'PRESCRIPTION',
        display: `Prescription ${displayDate(
          prescription.issue_date || prescription.createdAt
        )}`,
  ...
  for (const report of labs) {
    contexts.push(
      await upsertContext({
        ...
        display: `Lab Report ${report.report_type || ''} ${displayDate(
          report.report_date || report.createdAt
        )}`.trim(),
  ...
  for (const report of radiology) {
    contexts.push(
      await upsertContext({
        patient,
        hiType: 'DIAGNOSTIC_REPORT',
        display: `Radiology ${report.testName || ''} ${displayDate(
          report.requestedDate || report.createdAt
        )}`.trim(),
  ...
  for (const summary of discharges) {
    contexts.push(
      await upsertContext({
        ...
        display: `Discharge Summary ${displayDate(
          summary.dischargeDate || summary.createdAt
        )}`,
  ...
  for (const invoice of invoices) {
    contexts.push(
      await upsertContext({
        ...
        hiType: 'INVOICE',
        display: `Invoice ${
          invoice.invoice_number || invoice.invoiceNumber || invoice._id
        }`,
  ...
  for (const immunization of immunizations) {
    contexts.push(
      await upsertContext({
        ...
        display: `Immunization ${immunization.vaccineName} ${displayDate(
  ...
  for (const vital of opdVitals) {
    contexts.push(
      await upsertContext({
        ...
        display: `OPD Vitals ${displayDate(
          vital.recorded_at || vital.createdAt
        )}`,
  ...
  for (const vital of ipdVitals) {
    contexts.push(
      await upsertContext({
        patient,
        hiType: 'WELLNESS_RECORD',
        display: `IPD Vitals ${displayDate(
          vital.recordedAt || vital.createdAt
        )}`,
...
async function groupedForAbdm(patientId) {
  ...
  if (!patient.hospitalId) {
    throw new Error(
      'Patient hospitalId is required before reading ABDM care contexts'
    );
  }
  assertAbdmExchangeEligible(patient);
