...
async function findItem({ hospitalId, rateCardId, externalCode, internalServiceModel, internalServiceId }) {
...
async function resolveStandardUnit(input) {
  if (input.standardAmount !== undefined && input.standardAmount !== null) return Number(input.standardAmount);
  if (input.rate !== undefined && input.rate !== null) return Number(input.rate);
  if (!input.internalServiceModel || !input.internalServiceId) return 0;
  const models = {
  ...
  if (!config) return 0;
  const row = await config.Model.findOne({ _id: input.internalServiceId, hospitalId: input.hospitalId }).lean();
  if (!row) throw httpError('Hospital service master record not found', 404, 'SERVICE_MASTER_NOT_FOUND');
  if (row.is_active === false || row.isActive === false || row.active === false) throw httpError('Hospital service is inactive', 409, 'SERVICE_MASTER_INACTIVE');
  if (row.is_billable === false || row.template_only === true) throw httpError('Selected record is not a billable service', 409, 'SERVICE_NOT_BILLABLE');
...
function selfQuote(input, standardAmount, quantity, explanation = ['Standard hospital cash rate selected']) {
  ...
  return {
    ...
    rateCard: null,
    inputs: { payer: 'SELF', serviceDate: new Date(input.serviceDate || Date.now()), quantity },
    amounts: {
...
function missingItemPolicy(rateCard, coverage, payer) {
  ...
  if (fromCard && fromCard !== 'inherit_payer') return fromCard;
...
function cashFallbackQuote({ input, standardUnit, quantity, coverage, rateCard, reason }) {
  ...
  return {
    ...
    serviceCode: input.externalCode || input.serviceCode || input.internalCode || null,
    rateCard: rateCard ? { id: rateCard._id, version: rateCard.version, name: rateCard.name } : null,
    inputs: {
    ...
    explanation: ['Service is not present in the payer rate card', 'Hospital SELF/cash rate applied as fallback'],
...
function nonAdmissibleQuote({ input, standardUnit, quantity, coverage, rateCard, reason }) {
  ...
  return {
    ...
    serviceCode: input.externalCode || input.serviceCode || input.internalCode || null,
    rateCard: rateCard ? { id: rateCard._id, version: rateCard.version, name: rateCard.name } : null,
    inputs: { payer: coverage?.payerId?.code || String(coverage?.payerId || ''), coverageId: coverage?._id, serviceDate: new Date(input.serviceDate || Date.now()), quantity },
    amounts: {
      hospitalStandard: total,
      contracted: total,
      eligible: 0,
    ...
    explanation: ['Payer contract marks the service as non-admissible', 'Patient liability retained unless an authorised concession is applied'],
...
function remainingPreauth(coverage) {
  ...
  if (!approved) return Number.POSITIVE_INFINITY;
...
function calculateAllocation({ contracted, standardAmount, item, coverage, input, rounding }) {
  ...
  let coPayPercentage = Number(input.coPayPercentage ?? coverage.beneficiary?.coPayPercentage ?? 0);
  let fixedPatientShare = 0;
  let sponsorCap = Number.POSITIVE_INFINITY;
  if (patientRule.mode === 'percentage') coPayPercentage = Number(patientRule.percentage || 0);
  if (patientRule.mode === 'fixed') fixedPatientShare = Number(patientRule.fixedAmount || 0);
  if (patientRule.mode === 'sponsor_cap') sponsorCap = Number(patientRule.sponsorCap || 0);
  if (patientRule.mode === 'patient_full') sponsorCap = 0;

  const contractCap = Number(input.sponsorApprovalCap || Number.POSITIVE_INFINITY);
  const eligible = round(Math.min(contracted, sponsorCap, contractCap, remainingCoverageLimit(coverage), remainingPreauth(coverage)), rounding);
  ...
  const deductibleRemaining = Math.max(0, Number(input.deductibleRemaining ?? Math.max(0, Number(coverage.beneficiary?.deductibleAmount || 0) - Number(coverage.beneficiary?.deductibleUsed || 0))));
  const deductible = round(Math.min(Math.max(0, eligible - coPay - patientFixed), deductibleRemaining), rounding);
  ...
  if (uncovered > 0) {
    if (balancePolicy === 'patient') patientLiability = round(patientLiability + uncovered, rounding);
    else if (balancePolicy === 'hospital_concession' || balancePolicy === 'not_allowed') hospitalConcession = uncovered;
    else if (balancePolicy === 'requires_approval') {
      if (!input.balanceBillingApproved) throw httpError('Uncovered payer amount requires an authorised allocation decision', 409, 'BALANCE_BILLING_APPROVAL_REQUIRED', { uncovered });
      if (input.approvedUncoveredTreatment === 'patient') patientLiability = round(patientLiability + uncovered, rounding);
      else hospitalConcession = uncovered;
    }
  }
...
async function quotePricing(input) {
  ...
  if (!coverage || coverage.payerCategory === 'self' || coverage.payerId?.type === 'self') {
    return selfQuote(input, standardUnit, quantity, ['Standard hospital cash rate selected', coverage ? 'SELF coverage selected' : 'No sponsor coverage selected']);
  }
  ...
  const payer = coverage.payerId?.pricingPolicy ? coverage.payerId : await Payer.findOne({ _id: coverage.payerId, hospitalId: input.hospitalId });
  const rateCard = coverage.rateCardId
    ? coverage.rateCardId.rules
      ? coverage.rateCardId
      : await resolveEffectiveRateCard({ hospitalId: input.hospitalId, payerId: payer?._id || coverage.payerId, serviceDate, explicitRateCardId: coverage.rateCardId?._id || coverage.rateCardId, allowDemo: coverage.simulationOnly })
    : await resolveEffectiveRateCard({ hospitalId: input.hospitalId, payerId: payer?._id || coverage.payerId, serviceDate, allowDemo: coverage.simulationOnly });

  if (!rateCard) {
    const policy = coverage.fallbackPolicy || payer?.pricingPolicy?.missingItem || 'cash_fallback';
    if (policy === 'cash_fallback') return cashFallbackQuote({ input, standardUnit, quantity, coverage, rateCard: null, reason: 'No active payer rate card; hospital cash price applied' });
  ...
  const packageDecision = input.skipPackageAdjudication ? null : await findActivePackageDecision({
    ...input,
    coverage,
    serviceType,
    serviceDate,
  ...
  if (packageDecision?.decision === 'included') {
    ...
    return {
      ...
      inputs: { payer: payer?.code || payer?.name || String(coverage.payerId), coverageId: coverage._id, serviceDate, quantity },
      amounts: {
        hospitalStandard: total,
        contracted: 0,
        eligible: 0,
      ...
      explanation: [`Included in active package ${packageDecision.episode.packageCode}`, packageDecision.reason],
      ruleTrace: [{ rule: 'package_inclusion', packageEpisodeId: packageDecision.episode._id }]
    };
  }
  ...
  if (!item) {
    ...
    if (policy === 'cash_fallback') return cashFallbackQuote({ input, standardUnit, quantity, coverage, rateCard });
    if (policy === 'non_admissible') return nonAdmissibleQuote({ input, standardUnit, quantity, coverage, rateCard });
    throw httpError('Service is not mapped to the selected payer rate card', 422, 'PAYER_ITEM_NOT_FOUND');
  }
  if (item.pricingMode === 'non_admissible') return nonAdmissibleQuote({ input, standardUnit, quantity, coverage, rateCard, reason: `Payer item ${item.externalCode} is non-admissible` });

  if (item.allowedWards?.length && !item.allowedWards.includes(input.wardEntitlement || coverage.beneficiary?.wardEntitlement || 'semi_private')) {
    return nonAdmissibleQuote({ input, standardUnit, quantity, coverage, rateCard, reason: 'Service is not allowed for the beneficiary ward entitlement' });
  }
  ...
  const allocation = calculateAllocation({ contracted, standardAmount, item, coverage, input, rounding: rules.rounding });

  return {
    resultType: ['excluded', 'limit_exceeded'].includes(packageDecision?.decision) ? `package_${packageDecision.decision}` : 'payer_rate',
    serviceCode: item.externalCode,
    rateCard: { id: rateCard._id, version: rateCard.version, name: rateCard.name },
    rateCardItemId: item._id,
    packageCode: item.packageDefinition?.isPackage ? item.externalCode : packageDecision?.episode?.packageCode,
    ...
    explanation: [
      ...
      allocation.uncovered > 0 ? `${allocation.uncovered} uncovered amount handled by ${allocation.balancePolicy}` : 'No uncovered contracted amount',
      ...(['excluded', 'limit_exceeded'].includes(packageDecision?.decision) ? [`${packageDecision.decision === 'limit_exceeded' ? 'Package limit exceeded for' : 'Excluded from'} active package ${packageDecision.episode.packageCode}`] : [])
    ],
    ruleTrace: [
      ...applied.ruleTrace,
      { rule: 'liability_allocation', eligible: allocation.eligible, coPay: allocation.coPay, deductible: allocation.deductible, uncovered: allocation.uncovered, balancePolicy: allocation.balancePolicy }
    ]
  };
}
...
module.exports = { quotePricing, serviceTypeFromCharge, round, pricingSnapshot, findItem, resolveStandardUnit, calculateAllocation };
