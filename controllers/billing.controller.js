...
// ========== HELPER: Create/Update IPD Charge ==========
async function createOrUpdateIPDCharge({
  ...
  // Check if charge already exists for this source
  let existingCharge = await IPDCharge.findOne({
    admissionId,
    sourceModule,
    sourceId,
    isBilled: false
  ...
  if (existingCharge) {
    ...
    existingCharge.quantity = quantity || 1;
    existingCharge.rate = rate || 0;
...
// ========== HELPER: Mark IPD Charge as Billed ==========
async function markIPDChargeAsBilled(admissionId, sourceModule, sourceId, invoiceId, invoiceNumber) {
  if (!admissionId) return null;
  ...
  if (ipdCharge) {
    ...
    const IPDBillingController = require('./ipdBilling.controller');
    await IPDBillingController.updateAdmissionTotals(admissionId);
...
// ========== HELPER: Update Admission Totals ==========
      ...
      if (admission_id) {
        for (const item of items) {
          ...
          if (sourceModule && sourceId) {
            await markIPDChargeAsBilled(admission_id, sourceModule, sourceId, invoice._id, invoice.invoice_number);
          }
        }
      }
    ...
    if (isBecomingPaid && !updatedBill.invoice_id) {
      ...
      if (updatedBill.admission_id) {
        for (const item of updatedBill.items) {
          ...
          if (sourceModule && sourceId) {
            await markIPDChargeAsBilled(updatedBill.admission_id, sourceModule, sourceId, invoice._id, invoice.invoice_number);
          }
        }
      }
