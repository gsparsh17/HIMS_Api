...
exports.getAllPrescriptions = async (req, res) => {
  try {
    // Step 1: Get prescriptions and join with their items
    let prescriptions = await Prescription.aggregate([
      {
        $lookup: {
    ...
    // --- LOG 1: DATA BEFORE POPULATE ---
    console.log('--- Data AFTER $lookup (Before Populate) ---');
    console.log(JSON.stringify(prescriptions, null, 2));

    // Step 2: Populate all the references
    prescriptions = await Prescription.populate(prescriptions, [
        { path: 'patient_id' },
        { path: 'doctor_id' },
        { 
    ...
    // --- LOG 2: DATA AFTER POPULATE ---
    console.log('--- Data AFTER Populate ---');
    console.log(JSON.stringify(prescriptions, null, 2));
