...
medicineBatchSchema.pre('validate', async function(next) {
  try {
    let medicine = null;
    if (!this.units_per_pack || this.units_per_pack < 1 || !this.hsn_code || this.gst_rate == null) {
      const Medicine = mongoose.model('Medicine');
      medicine = await Medicine.findById(this.medicine_id).select('units_per_pack hsn_code gst_rate');
    }
    if (!this.units_per_pack || this.units_per_pack < 1) this.units_per_pack = medicine?.units_per_pack || 1;
    if (!this.hsn_code && medicine?.hsn_code) this.hsn_code = medicine.hsn_code;
    if (this.gst_rate == null && medicine?.gst_rate != null) this.gst_rate = medicine.gst_rate;

    if (this.quantity_base_units == null) this.quantity_base_units = this.quantity || 0;
