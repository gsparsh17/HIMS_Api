...
const templates = [
  ...
  {
    id: 'blood_transfusion_adverse_effect', version: 3,
    title: 'Blood Transfusion Notes and Reaction Form', shortTitle: 'Blood Transfusion Record', category: 'transfusion', stage: 'ongoing', required: false,
    implementation: 'structured', rendererId: 'blood-transfusion-record', pageCount: 2, referencePages: ['Blood Transfusion Flow Sheet.pdf p1-2'],
    description: 'Doctor order, nurse monitoring and transfusion-reaction record matching the supplied reference form.',
    sections: [
      section('Part A - To be filled by Doctor', [
        field('diagnosis', 'Diagnosis', 'textarea', { required: true }),
        field('patientBloodGroup', 'Patient blood group', 'text', { required: true }),
        field('bloodComponent', 'Blood component', 'select', { required: true, options: ['Whole Blood', 'Packed Red Cells', 'Platelets', 'FFP', 'Cryoprecipitate', 'Other'] }),
        field('bloodBagNumber', 'Blood bag number', 'text', { required: true }),
        field('unitNumber', 'Unit number'),
        field('collectionDate', 'Date of collection', 'date'),
        field('donationDate', 'Date of donation', 'date'),
        field('expiryDate', 'Expiry date', 'date', { required: true }),
        field('amountMl', 'Amount (ml)'),
        field('plannedHours', 'Planned duration (hours)'),
        field('dropsPerMinute', 'Drops/minute'),
        field('doctorName', 'Name of doctor', 'text', { required: true }),
        field('nurseName', 'Name of nurse', 'text', { required: true }),
      ]),
      section('Part B - Blood Transfusion Monitoring', [
        field('consentTaken', 'Consent taken', 'select', { required: true, options: ['Yes', 'No'] }),
        field('transfusionDate', 'Date', 'date', { required: true }),
        field('startTime', 'Time started', 'time', { required: true }),
        field('endTime', 'Time ended', 'time'),
        field('preTransfusionVitals', 'Vitals before commencement', 'table', { columns: [field('temperature', 'Temperature'), field('pulse', 'Pulse'), field('bp', 'BP'), field('rr', 'RR'), field('other', 'Other')] }),
        field('monitoringRows', 'Monitoring observations', 'table', { required: true, columns: [field('time', 'Time', 'time'), field('dropsPerMinute', 'Drops/min'), field('temperature', 'Temp'), field('pulse', 'Pulse/min'), field('respiratoryRate', 'Resp. rate'), field('bp', 'BP (mm Hg)'), field('allergicReaction', 'Allergic reaction / remarks')] }),
        field('completionRemarks', 'Completion remarks', 'textarea'),
      ]),
      section('Blood Transfusion Reaction Form', [
        field('consultant', 'Consultant'),
        field('departmentUnit', 'Department and unit'),
        field('reactionDiagnosis', 'Diagnosis'),
        field('reactionDate', 'Date', 'date'),
        field('reactionComponent', 'Component'),
        field('reactionUnitNumber', 'Unit number'),
        field('reactionExpiryDate', 'Expiry date', 'date'),
        field('bloodBank', 'Blood bank'),
        field('reactionTransfusionStartTime', 'Transfusion start time', 'time'),
        field('reactionTime', 'Reaction time', 'time'),
        field('volumeTransfused', 'Volume transfused'),
        field('clericalChecks', 'Clerical check', 'table', { defaultRows: [{ item: 'Patient ID correct' }, { item: 'Blood pack correct' }, { item: 'Blood transfusion record correct' }], columns: [field('item', 'Check'), field('status', 'Status', 'select', { options: ['Yes', 'No'] }), field('remarks', 'Remarks')] }),
        field('febrileStatus', 'Temperature in prior 24 hours', 'select', { options: ['Febrile', 'Afebrile'] }),
        field('reactionVitals', 'Reaction vital signs', 'table', { columns: [field('stage', 'Stage', 'select', { options: ['Pre Reaction', 'Post Reaction'] }), field('time', 'Time'), field('temperature', 'Temperature'), field('respiratoryRate', 'Respiratory rate'), field('bp', 'BP'), field('pulseRate', 'Pulse rate')] }),
        field('reactionSymptoms', 'Signs and symptoms', 'checklist', { options: ['Fever', 'Chills', 'Nausea / Vomiting', 'Hives / Itching', 'Other', 'Lower back pain', 'Chest pain', 'Anxiety', 'Headache', 'Skin pallor', 'Dark urine', 'Dyspnoea', 'Bleeding from wound / IV site'] }),
        field('previousBloodProducts', 'Blood products given in previous 12 hours', 'table', { columns: [field('donorUnitNumber', 'Donor unit number'), field('productType', 'Product type'), field('date', 'Date', 'date'), field('time', 'Time', 'time'), field('units', 'Units'), field('volume', 'Volume'), field('reaction', 'Reaction')] }),
        field('reactionActions', 'Immediate action / treatment', 'textarea'),
        field('samplesSent', 'Sealed blood bag, recipient blood sample and first voided urine sent to blood bank', 'checkbox'),
        field('reactionOutcome', 'Outcome and further plan', 'textarea'),
        field('reviewingDoctorName', 'Name of reviewing doctor', 'text'),
        field('reviewedAt', 'Review date and time', 'datetime-local'),
      ]),
    ],
  },
  {
